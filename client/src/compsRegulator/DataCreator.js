...
//     "Use the guided sections for the most common generators.",
//     "Use advanced overrides for rarer fields such as fuel metrics, household income, or custom arrays.",
//     "The backend still relies on a Flask service at `http://127.0.0.1:5000/synthic`, and this page sends a cleaned payload through the Node server proxy.",
//     "Derived fields such as working children, total household income, and vehicle or fuel columns can now be auto-generated by the Flask service when a downstream rule needs them."
// ];
...
//         },
//         vehicleOwnership: {
//             pvalue: "0.2, 0.8"                    // P(no vehicle), P(has vehicle)
//         },
...
//             cyl4: "100",                           // litres for 4-cylinder
//             cyl6: "130",                           // litres for 6-cylinder
//             cyl8: "160"                            // litres for 8-cylinder
//         },
...
//             p_young: "0.1",                        // P(child works) when applicant age < 35
//             p_old: "0.3"                           // P(child works) when applicant age >= 35
//         },
...
//     if (nEligible !== null) {
//         payload.n_eligible = nEligible;
//     }

//     if (nIneligible !== null) {
//         payload.n_ineligible = nIneligible;
//     }

//     if (fraudFraction !== null) {
//         payload.fraud_fraction = fraudFraction;
//     }
...
//         const minAge = parseRequiredNumber(state.age.minAge, "Minimum age", errors, {
//             integer: true,
//             min: 0
//         });
//         const maxAge = parseRequiredNumber(state.age.maxAge, "Maximum age", errors, {
//             integer: true,
//             min: 0
//         });
...
//         if ([center, scale, minAge, maxAge].every((value) => value !== null)) {
//             payload.Age = {
//                 Center: center,
//                 Scale: scale,
//                 minAge,
...
//         if (pValue) {
//             payload.Gender = { pValue };
//         }
//     }

//     if (state.sections.Marital_Status) {
//         const pValue = parseProbabilityList(
//             state.marital.pValue,
//             "Marital status probabilities",
//             4,
//             errors
//         );
//         if (pValue) {
//             payload.Marital_Status = { pValue };
//         }
//     }
...
//         if ([lam, minHouseholdSize, maxHouseholdSize].every((value) => value !== null)) {
//             payload.Household_Size = {
//                 lam,
//                 MinHsize: minHouseholdSize,
//                 MaxHsize: maxHouseholdSize
//             };
//         }
//     }
...
//         const escale = parseRequiredNumber(state.salary.escale, "Eligible salary scale", errors, {
//             min: 0
//         });
//         const inloc = parseRequiredNumber(state.salary.inloc, "Ineligible salary mean", errors);
//         const inscale = parseRequiredNumber(
//             state.salary.inscale,
//             "Ineligible salary scale",
//             errors,
//             { min: 0 }
//         );
//         const minSalary = parseRequiredNumber(state.salary.minSalary, "Minimum salary", errors, {
//             min: 0
//         });

//         if ([eloc, escale, inloc, inscale, minSalary].every((value) => value !== null)) {
//             payload.Salary = {
//                 eloc,
//                 escale,
//                 inloc,
...
//         const pvalue = parseProbabilityList(
//             state.degreeLevel.pvalue,
//             "Degree level probabilities",
//             5,
//             errors
//         );
//         if (pvalue) {
//             payload.Degree_Level = { pvalue };
//         }
//     }

//     if (state.sections.Employment_Status) {
//         const pvalue = parseProbabilityList(
//             state.employment.pvalue,
//             "Employment status probabilities",
//             4,
//             errors
//         );
//         if (pvalue) {
//             payload.Employment_Status = { pvalue };
//         }
//     }

//     if (state.sections.Primary_Income_Source) {
//         const pvalue = parseProbabilityList(
//             state.primaryIncomeSource.pvalue,
//             "Primary income source probabilities",
//             4,
//             errors
//         );
//         if (pvalue) {
//             payload.Primary_Income_Source = { pvalue };
//         }
//     }

//     if (state.sections.Has_Other_Social_Benefits) {
//         const pvalue = parseProbabilityList(
//             state.otherBenefits.pvalue,
//             "Other social benefits probabilities",
//             2,
//             errors
//         );
//         if (pvalue) {
//             payload.Has_Other_Social_Benefits = { pvalue };
//         }
//     }
...
//         if ([eligibleMean, eligibleSigma, ineligibleMean, ineligibleSigma].every((value) => value !== null)) {
...
//         if ([mean, sigma].every((value) => value !== null)) {
...
//         if ([mean, sigma, max].every((value) => value !== null)) {
...
//         payload.Vehicle_Ownership = true;
//     }

//     if (state.sections.Vehicle_Count) {
//         payload.Vehicle_Count = true;
//     }

//     if (state.sections.Cylinder_Count) {
//         payload.Cylinder_Count = true;
//     }

//     if (state.sections.Vehicle_Age_Years) {
//         payload.Vehicle_Age_Years = true;
//     }

//     if (state.sections.Fuel_Type) {
//         payload.Fuel_Type = true;
//     }

//     if (state.sections.Expected_Fuel_Consumption_L) {
//         payload.Expected_Fuel_Consumption_L = true;
//     }

//     if (state.sections.Average_Fuel_Consumption_L) {
//         payload.Average_Fuel_Consumption_L = true;
//     }
...
//         const pvalue = parseProbabilityList(
//             state.previousSubsidy.pvalue,
//             "Previous subsidy probabilities",
//             2,
//             errors
//         );
//         const amountMean = parseRequiredNumber(
//             state.previousSubsidy.amount_mean,
//             "Previous subsidy amount mean",
//             errors
//         );
//         const amountStd = parseRequiredNumber(
//             state.previousSubsidy.amount_std,
//             "Previous subsidy amount standard deviation",
//             errors,
//             { min: 0 }
//         );
//         const amountMin = parseRequiredNumber(
//             state.previousSubsidy.amount_min,
//             "Previous subsidy minimum amount",
//             errors,
//             { min: 0 }
//         );
//         const amountMax = parseRequiredNumber(
//             state.previousSubsidy.amount_max,
//             "Previous subsidy maximum amount",
//             errors,
//             { min: 0 }
//         );
...
//         if (
//             pvalue &&
//             [amountMean, amountStd, amountMin, amountMax].every((value) => value !== null)
//         ) {
...
//         if (lam !== null) {
//             payload.Late_or_Missed_Renewals = { lam };
//         }
//     }
...
//         if (lam !== null) {
//             payload.Applications_Last_12_Months = { lam };
//         }
//     }
...
//         const sectionFraudFraction = parseRequiredNumber(
//             state.fraudProfile.fraud_fraction,
//             "Fraud profile fraction",
//             errors,
//             { min: 0.000001, max: 0.999999, allowZero: false }
//         );
//         const fuelMultiplier = parseRequiredNumber(
//             state.fraudProfile.fuel_multiplier,
//             "Fuel multiplier",
//             errors,
//             { min: 0.1, allowZero: false }
//         );
//         const appsBoost = parseRequiredNumber(
//             state.fraudProfile.apps_boost,
//             "Applications boost",
//             errors,
//             { integer: true, min: 0 }
//         );
//         const renewalsBoost = parseRequiredNumber(
//             state.fraudProfile.renewals_boost,
//             "Renewals boost",
//             errors,
//             { integer: true, min: 0 }
//         );
...
//             <CardHeader
//                 style={{
//                     background: "linear-gradient(135deg, rgba(102, 187, 106, 0.2), rgba(102, 187, 106, 0.06))",
...
//             toast.error("Fix validation errors before copying the payload.");
//             return;
//         }
...
//         if (serialized.errors.length) {
//             toast.error("Fix validation errors before downloading the payload.");
//             return;
//         }
...
//         if (serialized.errors.length) {
//             toast.error("Fix validation errors before generating data.");
//             return;
//         }
...
//                         </CardTitle>
//                         <ListGroup flush>
//                             {fieldDescriptions.map((line) => (
//                                 <ListGroupItem
//                                     key={line}
//                                     style={{
//                                         backgroundColor: "transparent",
//                                         color: theme.textColorAlt,
//                                         borderColor: "rgba(102, 187, 106, 0.12)"
//                                     }}
//                                 >
//                                     {line}
//                                 </ListGroupItem>
...
//                         <Row className="g-3">
//                             <Col md="4"><FormGroup><Label for="nEligible" style={{ color: theme.textColorAlt }}>Number of eligible records</Label><Input id="nEligible" type="number" value={formState.basics.n_eligible} onChange={(event) => updateBasics("n_eligible", event.target.value)} style={inputStyle} /></FormGroup></Col>
//                             <Col md="4"><FormGroup><Label for="nIneligible" style={{ color: theme.textColorAlt }}>Number of ineligible records</Label><Input id="nIneligible" type="number" value={formState.basics.n_ineligible} onChange={(event) => updateBasics("n_ineligible", event.target.value)} style={inputStyle} /></FormGroup></Col>
//                             <Col md="4"><FormGroup><Label for="fraudFraction" style={{ color: theme.textColorAlt }}>Top-level fraud fraction</Label><Input id="fraudFraction" type="number" step="0.01" value={formState.basics.fraud_fraction} onChange={(event) => updateBasics("fraud_fraction", event.target.value)} style={inputStyle} /></FormGroup></Col>
//                         </Row>
//                     </InfoCard>

//                     <InfoCard title="Common Field Generators" theme={theme}>
//                         <div className="d-flex gap-2 flex-wrap mb-3">
//                             <Button color="success" size="sm" onClick={() => setSectionsState(commonSectionKeys, true)}>
//                                 Select All
//                             </Button>
//                             <Button color="secondary" size="sm" onClick={() => setSectionsState(commonSectionKeys, false)}>
//                                 Clear All
//                             </Button>
//                         </div>
//                         {renderSectionCard("Age generator", "ageToggle", formState.sections.Age, () => toggleSection("Age"), <Row className="g-3"><Col md="3"><Label for="ageCenter" style={{ color: theme.textColorAlt }}>Center</Label><Input id="ageCenter" type="number" value={formState.age.Center} onChange={(event) => updateSection("age", "Center", event.target.value)} style={inputStyle} /></Col><Col md="3"><Label for="ageScale" style={{ color: theme.textColorAlt }}>Scale</Label><Input id="ageScale" type="number" value={formState.age.Scale} onChange={(event) => updateSection("age", "Scale", event.target.value)} style={inputStyle} /></Col><Col md="3"><Label for="minAge" style={{ color: theme.textColorAlt }}>Minimum age</Label><Input id="minAge" type="number" value={formState.age.minAge} onChange={(event) => updateSection("age", "minAge", event.target.value)} style={inputStyle} /></Col><Col md="3"><Label for="maxAge" style={{ color: theme.textColorAlt }}>Maximum age</Label><Input id="maxAge" type="number" value={formState.age.maxAge} onChange={(event) => updateSection("age", "maxAge", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Gender generator", "genderToggle", formState.sections.Gender, () => toggleSection("Gender"), <FormGroup><Label for="genderPValue" style={{ color: theme.textColorAlt }}>Probabilities for Male, Female</Label><Input id="genderPValue" value={formState.gender.pValue} onChange={(event) => updateSection("gender", "pValue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Marital status generator", "maritalToggle", formState.sections.Marital_Status, () => toggleSection("Marital_Status"), <FormGroup><Label for="maritalPValue" style={{ color: theme.textColorAlt }}>Probabilities for Single, Married, Divorced, Widowed</Label><Input id="maritalPValue" value={formState.marital.pValue} onChange={(event) => updateSection("marital", "pValue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Governorate generator", "governorateToggle", formState.sections.Governorate, () => toggleSection("Governorate"), <Row className="g-3"><Col md="6"><Label for="cities" style={{ color: theme.textColorAlt }}>Cities</Label><Input id="cities" type="textarea" rows="3" value={formState.governorate.City} onChange={(event) => updateSection("governorate", "City", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="cityWeights" style={{ color: theme.textColorAlt }}>Probabilities</Label><Input id="cityWeights" type="textarea" rows="3" value={formState.governorate.PvalueSize} onChange={(event) => updateSection("governorate", "PvalueSize", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Household size generator", "householdSizeToggle", formState.sections.Household_Size, () => toggleSection("Household_Size"), <Row className="g-3"><Col md="4"><Label for="householdLambda" style={{ color: theme.textColorAlt }}>Lambda</Label><Input id="householdLambda" type="number" value={formState.householdSize.lam} onChange={(event) => updateSection("householdSize", "lam", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="householdMin" style={{ color: theme.textColorAlt }}>Minimum size</Label><Input id="householdMin" type="number" value={formState.householdSize.MinHsize} onChange={(event) => updateSection("householdSize", "MinHsize", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="householdMax" style={{ color: theme.textColorAlt }}>Maximum size</Label><Input id="householdMax" type="number" value={formState.householdSize.MaxHsize} onChange={(event) => updateSection("householdSize", "MaxHsize", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Salary generator", "salaryToggle", formState.sections.Salary, () => toggleSection("Salary"), <Row className="g-3"><Col md="4"><Label for="eligibleMean" style={{ color: theme.textColorAlt }}>Eligible mean</Label><Input id="eligibleMean" type="number" value={formState.salary.eloc} onChange={(event) => updateSection("salary", "eloc", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="eligibleScale" style={{ color: theme.textColorAlt }}>Eligible scale</Label><Input id="eligibleScale" type="number" value={formState.salary.escale} onChange={(event) => updateSection("salary", "escale", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="minimumSalary" style={{ color: theme.textColorAlt }}>Minimum salary</Label><Input id="minimumSalary" type="number" value={formState.salary.minSalary} onChange={(event) => updateSection("salary", "minSalary", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="ineligibleMean" style={{ color: theme.textColorAlt }}>Ineligible mean</Label><Input id="ineligibleMean" type="number" value={formState.salary.inloc} onChange={(event) => updateSection("salary", "inloc", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="ineligibleScale" style={{ color: theme.textColorAlt }}>Ineligible scale</Label><Input id="ineligibleScale" type="number" value={formState.salary.inscale} onChange={(event) => updateSection("salary", "inscale", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Degree level generator", "degreeLevelToggle", formState.sections.Degree_Level, () => toggleSection("Degree_Level"), <FormGroup><Label for="degreeLevelWeights" style={{ color: theme.textColorAlt }}>Probabilities for None, HighSchool, Diploma, Bachelor, Master</Label><Input id="degreeLevelWeights" value={formState.degreeLevel.pvalue} onChange={(event) => updateSection("degreeLevel", "pvalue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Employment status generator", "employmentToggle", formState.sections.Employment_Status, () => toggleSection("Employment_Status"), <FormGroup><Label for="employmentWeights" style={{ color: theme.textColorAlt }}>Probabilities for Employed, Unemployed, Student, Retired</Label><Input id="employmentWeights" value={formState.employment.pvalue} onChange={(event) => updateSection("employment", "pvalue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Primary income source generator", "primaryIncomeToggle", formState.sections.Primary_Income_Source, () => toggleSection("Primary_Income_Source"), <FormGroup><Label for="primaryIncomeWeights" style={{ color: theme.textColorAlt }}>Probabilities for Government, Private, SelfEmployed, None</Label><Input id="primaryIncomeWeights" value={formState.primaryIncomeSource.pvalue} onChange={(event) => updateSection("primaryIncomeSource", "pvalue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Other social benefits generator", "otherBenefitsToggle", formState.sections.Has_Other_Social_Benefits, () => toggleSection("Has_Other_Social_Benefits"), <FormGroup><Label for="otherBenefitsWeights" style={{ color: theme.textColorAlt }}>Probabilities for 0, 1</Label><Input id="otherBenefitsWeights" value={formState.otherBenefits.pvalue} onChange={(event) => updateSection("otherBenefits", "pvalue", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Assets value generator", "assetsValueToggle", formState.sections.Assets_Value, () => toggleSection("Assets_Value"), <Row className="g-3"><Col md="6"><Label for="eligibleAssetsMean" style={{ color: theme.textColorAlt }}>Eligible mean</Label><Input id="eligibleAssetsMean" type="number" value={formState.assetsValue.eligible_mean} onChange={(event) => updateSection("assetsValue", "eligible_mean", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="eligibleAssetsSigma" style={{ color: theme.textColorAlt }}>Eligible sigma</Label><Input id="eligibleAssetsSigma" type="number" value={formState.assetsValue.eligible_sigma} onChange={(event) => updateSection("assetsValue", "eligible_sigma", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="ineligibleAssetsMean" style={{ color: theme.textColorAlt }}>Ineligible mean</Label><Input id="ineligibleAssetsMean" type="number" value={formState.assetsValue.ineligible_mean} onChange={(event) => updateSection("assetsValue", "ineligible_mean", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="ineligibleAssetsSigma" style={{ color: theme.textColorAlt }}>Ineligible sigma</Label><Input id="ineligibleAssetsSigma" type="number" value={formState.assetsValue.ineligible_sigma} onChange={(event) => updateSection("assetsValue", "ineligible_sigma", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Liabilities value generator", "liabilitiesValueToggle", formState.sections.Liabilities_Value, () => toggleSection("Liabilities_Value"), <Row className="g-3"><Col md="6"><Label for="liabilitiesMean" style={{ color: theme.textColorAlt }}>Mean</Label><Input id="liabilitiesMean" type="number" value={formState.liabilitiesValue.mean} onChange={(event) => updateSection("liabilitiesValue", "mean", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="liabilitiesSigma" style={{ color: theme.textColorAlt }}>Sigma</Label><Input id="liabilitiesSigma" type="number" value={formState.liabilitiesValue.sigma} onChange={(event) => updateSection("liabilitiesValue", "sigma", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Number of children generator", "numberOfChildrenToggle", formState.sections.Number_of_Children, () => toggleSection("Number_of_Children"), <Row className="g-3"><Col md="6"><Label for="childrenLambda" style={{ color: theme.textColorAlt }}>Lambda</Label><Input id="childrenLambda" type="number" value={formState.numberOfChildren.lam} onChange={(event) => updateSection("numberOfChildren", "lam", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="childrenMax" style={{ color: theme.textColorAlt }}>Maximum children</Label><Input id="childrenMax" type="number" value={formState.numberOfChildren.max} onChange={(event) => updateSection("numberOfChildren", "max", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Spouse income generator", "spouseIncomeToggle", formState.sections.Total_Spouse_Income, () => toggleSection("Total_Spouse_Income"), <Row className="g-3"><Col md="4"><Label for="spouseIncomeMean" style={{ color: theme.textColorAlt }}>Mean</Label><Input id="spouseIncomeMean" type="number" value={formState.spouseIncome.mean} onChange={(event) => updateSection("spouseIncome", "mean", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="spouseIncomeSigma" style={{ color: theme.textColorAlt }}>Sigma</Label><Input id="spouseIncomeSigma" type="number" value={formState.spouseIncome.sigma} onChange={(event) => updateSection("spouseIncome", "sigma", event.target.value)} style={inputStyle} /></Col><Col md="4"><Label for="spouseIncomeMax" style={{ color: theme.textColorAlt }}>Maximum</Label><Input id="spouseIncomeMax" type="number" value={formState.spouseIncome.max} onChange={(event) => updateSection("spouseIncome", "max", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Children income generator", "childrenIncomeToggle", formState.sections.Total_Children_Income, () => toggleSection("Total_Children_Income"), <Row className="g-3"><Col md="6"><Label for="childrenIncomeMean" style={{ color: theme.textColorAlt }}>Mean</Label><Input id="childrenIncomeMean" type="number" value={formState.childrenIncome.mean} onChange={(event) => updateSection("childrenIncome", "mean", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="childrenIncomeSigma" style={{ color: theme.textColorAlt }}>Sigma</Label><Input id="childrenIncomeSigma" type="number" value={formState.childrenIncome.sigma} onChange={(event) => updateSection("childrenIncome", "sigma", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Previous subsidy generator", "previousToggle", formState.sections.Previous_Subsidy_Received, () => toggleSection("Previous_Subsidy_Received"), <Row className="g-3"><Col md="6"><Label for="previousProbabilities" style={{ color: theme.textColorAlt }}>Probabilities for Not received, Received</Label><Input id="previousProbabilities" value={formState.previousSubsidy.pvalue} onChange={(event) => updateSection("previousSubsidy", "pvalue", event.target.value)} style={inputStyle} /></Col><Col md="3"><Label for="previousMean" style={{ color: theme.textColorAlt }}>Mean</Label><Input id="previousMean" type="number" value={formState.previousSubsidy.amount_mean} onChange={(event) => updateSection("previousSubsidy", "amount_mean", event.target.value)} style={inputStyle} /></Col><Col md="3"><Label for="previousStd" style={{ color: theme.textColorAlt }}>Std. dev.</Label><Input id="previousStd" type="number" value={formState.previousSubsidy.amount_std} onChange={(event) => updateSection("previousSubsidy", "amount_std", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="previousMin" style={{ color: theme.textColorAlt }}>Minimum amount</Label><Input id="previousMin" type="number" value={formState.previousSubsidy.amount_min} onChange={(event) => updateSection("previousSubsidy", "amount_min", event.target.value)} style={inputStyle} /></Col><Col md="6"><Label for="previousMax" style={{ color: theme.textColorAlt }}>Maximum amount</Label><Input id="previousMax" type="number" value={formState.previousSubsidy.amount_max} onChange={(event) => updateSection("previousSubsidy", "amount_max", event.target.value)} style={inputStyle} /></Col></Row>)}

//                         {renderSectionCard("Late or missed renewals generator", "lateRenewalsToggle", formState.sections.Late_or_Missed_Renewals, () => toggleSection("Late_or_Missed_Renewals"), <FormGroup><Label for="lateRenewalsLambda" style={{ color: theme.textColorAlt }}>Lambda</Label><Input id="lateRenewalsLambda" type="number" value={formState.lateRenewals.lam} onChange={(event) => updateSection("lateRenewals", "lam", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Applications in last 12 months generator", "applications12MonthsToggle", formState.sections.Applications_Last_12_Months, () => toggleSection("Applications_Last_12_Months"), <FormGroup><Label for="applications12MonthsLambda" style={{ color: theme.textColorAlt }}>Lambda</Label><Input id="applications12MonthsLambda" type="number" value={formState.applicationsLast12Months.lam} onChange={(event) => updateSection("applicationsLast12Months", "lam", event.target.value)} style={inputStyle} /></FormGroup>)}

//                         {renderSectionCard("Random ID generator", "idGeneratorToggle", formState.sections.ID, () => toggleSection("ID"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Enable this to let the backend populate an `ID` column with generated values.</CardText>)}
//                     </InfoCard>

//                     <InfoCard title="Derived And Vehicle Fields" theme={theme}>
//                         <CardText style={{ color: theme.textColorAlt }}>
//                             These route cases currently act as backend switches. Enabling one adds the matching top-level key to the payload so Flask will derive that column with its built-in formulas.
//                         </CardText>
//                         <div className="d-flex gap-2 flex-wrap mb-3">
//                             <Button color="success" size="sm" onClick={() => setSectionsState(derivedSectionKeys, true)}>
//                                 Select All
//                             </Button>
//                             <Button color="secondary" size="sm" onClick={() => setSectionsState(derivedSectionKeys, false)}>
//                                 Clear All
//                             </Button>
//                         </div>
//                         {renderSectionCard("Working children count", "workingChildrenToggle", formState.sections.Working_Children_Count, () => toggleSection("Working_Children_Count"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Working_Children_Count` from age and number of children defaults.</CardText>)}
//                         {renderSectionCard("Total household income", "totalHouseholdIncomeToggle", formState.sections.Total_Household_Income, () => toggleSection("Total_Household_Income"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Total_Household_Income` from salary, spouse income, and children income.</CardText>)}
//                         {renderSectionCard("Vehicle ownership", "vehicleOwnershipToggle", formState.sections.Vehicle_Ownership, () => toggleSection("Vehicle_Ownership"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate the `Vehicle_Ownership` indicator.</CardText>)}
//                         {renderSectionCard("Vehicle count", "vehicleCountToggle", formState.sections.Vehicle_Count, () => toggleSection("Vehicle_Count"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Vehicle_Count` from the backend vehicle rules.</CardText>)}
//                         {renderSectionCard("Cylinder count", "cylinderCountToggle", formState.sections.Cylinder_Count, () => toggleSection("Cylinder_Count"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Cylinder_Count` from the backend vehicle rules.</CardText>)}
//                         {renderSectionCard("Vehicle age in years", "vehicleAgeToggle", formState.sections.Vehicle_Age_Years, () => toggleSection("Vehicle_Age_Years"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Vehicle_Age_Years` from the backend defaults.</CardText>)}
//                         {renderSectionCard("Fuel type", "fuelTypeToggle", formState.sections.Fuel_Type, () => toggleSection("Fuel_Type"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Fuel_Type` from the backend defaults.</CardText>)}
//                         {renderSectionCard("Expected fuel consumption", "expectedFuelToggle", formState.sections.Expected_Fuel_Consumption_L, () => toggleSection("Expected_Fuel_Consumption_L"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Expected_Fuel_Consumption_L` from the backend formulas.</CardText>)}
//                         {renderSectionCard("Average fuel consumption", "averageFuelToggle", formState.sections.Average_Fuel_Consumption_L, () => toggleSection("Average_Fuel_Consumption_L"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Average_Fuel_Consumption_L` from the backend formulas.</CardText>)}
//                         {renderSectionCard("Fuel deviation in liters", "fuelDeviationToggle", formState.sections.Fuel_Deviation_L, () => toggleSection("Fuel_Deviation_L"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Fuel_Deviation_L` from the expected and average fuel columns.</CardText>)}
//                         {renderSectionCard("Fuel deviation ratio", "fuelDeviationRatioToggle", formState.sections.Fuel_Deviation_Ratio, () => toggleSection("Fuel_Deviation_Ratio"), <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>Generate `Fuel_Deviation_Ratio` from the expected and average fuel columns.</CardText>)}
//                     </InfoCard>
...
//                             </FormGroup>
//                             <Button color="success" onClick={addConditionGroup} disabled={!formState.sections.Conditions}><FaPlus className="me-2" />Add OR group</Button>
//                         </div>
//                         <CardText style={{ color: theme.textColorAlt }}>
...
//                                         <Button color="success" size="sm" onClick={() => addRuleToGroup(group.id)}><FaPlus className="me-2" />Add rule</Button>
//                                         <Button color="danger" size="sm" onClick={() => removeConditionGroup(group.id)}><FaTrash className="me-2" />Remove group</Button>
//                                     </div>
//                                 </div>

//                                 {group.rules.map((rule, ruleIndex) => (
//                                     <Row className="g-2 align-items-end mb-2" key={rule.id}>
//                                         <Col lg="4"><Label for={`${rule.id}-col`} style={{ color: theme.textColorAlt }}>Field</Label><Input id={`${rule.id}-col`} type="select" value={rule.col} onChange={(event) => updateConditionRule(group.id, rule.id, "col", event.target.value)} style={inputStyle}>{conditionFieldOptions.map((field) => <option key={field} value={field}>{field}</option>)}</Input></Col>
//                                         <Col lg="2"><Label for={`${rule.id}-op`} style={{ color: theme.textColorAlt }}>Operator</Label><Input id={`${rule.id}-op`} type="select" value={rule.op} onChange={(event) => updateConditionRule(group.id, rule.id, "op", event.target.value)} style={inputStyle}>{operatorOptions.map((option) => <option key={option} value={option}>{option}</option>)}</Input></Col>
//                                         <Col lg="5"><Label for={`${rule.id}-val`} style={{ color: theme.textColorAlt }}>Value</Label><Input id={`${rule.id}-val`} value={rule.val} onChange={(event) => updateConditionRule(group.id, rule.id, "val", event.target.value)} placeholder={rule.op === "between" ? "[18, 24]" : rule.op === "in" ? '["Student", "Unemployed"]' : "600"} style={inputStyle} /></Col>
//                                         <Col lg="1" className="d-flex justify-content-end"><Button color="danger" size="sm" onClick={() => removeRuleFromGroup(group.id, rule.id)} aria-label={`Remove rule ${ruleIndex + 1}`}><FaTrash /></Button></Col>
//                                     </Row>
//                                 ))}
//                             </div>
//                         ))}
...
//                         </FormGroup>
//                         <CardText style={{ color: theme.textColorAlt }}>
//                             This section boosts suspicious usage patterns. If the dependent vehicle and fuel columns are not supplied explicitly, the Flask service now backfills them using its default synthetic formulas before applying the fraud multipliers.
//                         </CardText>
//                         {formState.sections.fraudmulti && (
//                             <Row className="g-3">
//                                 <Col md="3"><Label for="fraudProfileFraction" style={{ color: theme.textColorAlt }}>Fraud fraction</Label><Input id="fraudProfileFraction" type="number" step="0.01" value={formState.fraudProfile.fraud_fraction} onChange={(event) => updateSection("fraudProfile", "fraud_fraction", event.target.value)} style={inputStyle} /></Col>
//                                 <Col md="3"><Label for="fuelMultiplier" style={{ color: theme.textColorAlt }}>Fuel multiplier</Label><Input id="fuelMultiplier" type="number" step="0.1" value={formState.fraudProfile.fuel_multiplier} onChange={(event) => updateSection("fraudProfile", "fuel_multiplier", event.target.value)} style={inputStyle} /></Col>
//                                 <Col md="3"><Label for="appsBoost" style={{ color: theme.textColorAlt }}>Applications boost</Label><Input id="appsBoost" type="number" value={formState.fraudProfile.apps_boost} onChange={(event) => updateSection("fraudProfile", "apps_boost", event.target.value)} style={inputStyle} /></Col>
//                                 <Col md="3"><Label for="renewalsBoost" style={{ color: theme.textColorAlt }}>Renewals boost</Label><Input id="renewalsBoost" type="number" value={formState.fraudProfile.renewals_boost} onChange={(event) => updateSection("fraudProfile", "renewals_boost", event.target.value)} style={inputStyle} /></Col>
//                             </Row>
//                         )}
//                     </InfoCard>

//                     <InfoCard title="Derived Backend Fields" theme={theme}>
//                         <CardText style={{ color: theme.textColorAlt }}>
//                             Some route fields are now generated automatically on the backend when they are needed as dependencies. This covers `Working_Children_Count`, `Total_Household_Income`, `Vehicle_Ownership`, `Vehicle_Count`, `Cylinder_Count`, `Vehicle_Age_Years`, `Fuel_Type`, `Expected_Fuel_Consumption_L`, `Average_Fuel_Consumption_L`, `Fuel_Deviation_L`, and `Fuel_Deviation_Ratio`.
//                         </CardText>
//                         <CardText style={{ color: theme.textColorAlt, marginBottom: 0 }}>
//                             You can still override any of them manually through Advanced Overrides if you want specific distributions instead of the backend defaults.
//                         </CardText>
//                     </InfoCard>
...
//                         </CardText>
//                         <Label for="advancedOverrides" style={{ color: theme.textColorAlt }}>
//                             Advanced Overrides
//                         </Label>
//                         <Input id="advancedOverrides" type="textarea" rows="10" value={formState.advancedOverrides} onChange={(event) => setFormState((current) => ({ ...current, advancedOverrides: event.target.value }))} placeholder={"{\n  \"Applications_Last_12_Months\": { \"lam\": 1.0 },\n  \"Late_or_Missed_Renewals\": { \"lam\": 0.3 },\n  \"Expected_Fuel_Consumption_L\": { \"mean\": 110, \"std\": 12, \"min\": 70, \"max\": 170 },\n  \"Average_Fuel_Consumption_L\": { \"mean\": 116, \"std\": 18, \"min\": 75, \"max\": 220 }\n}"} style={{ ...inputStyle, fontFamily: "Consolas, monospace" }} />
//                     </InfoCard>
//                 </Col>
...
//                             <Button color="primary" onClick={handleDownload} disabled={serialized.errors.length > 0}><FaDownload className="me-2" />Download JSON</Button>
//                         </div>
...
//                     <InfoCard title="Backend Response" theme={theme}>
//                         <CardText style={{ color: theme.textColorAlt }}>The Generate button posts the cleaned payload to the Node server proxy, which forwards it to the Flask /synthic route.</CardText>
//                         {serverResult ? (
//                             <pre style={{ backgroundColor: theme.primaryBackground, color: theme.textColorAlt, borderRadius: "12px", padding: "16px", minHeight: "220px", overflow: "auto", marginBottom: 0, border: "1px solid rgba(102, 187, 106, 0.15)" }}>{JSON.stringify(serverResult, null, 2)}</pre>
//                         ) : (
//                             <Alert color="info" className="mb-0">No generation result yet. Build the payload, then click Generate Dataset.</Alert>
//                         )}
//                     </InfoCard>
//                 </Col>
//             </Row>
        ...
        <Container className="py-4" style={{ minHeight: "80vh" }}>
            ...
            <Row className="g-4">
                <Col xl="7">
                    ...
                    <InfoCard title="Common Field Generators" theme={theme}>
                        <div className="d-flex gap-2 flex-wrap mb-3">
                            <Button color="success" size="sm" onClick={() => setSectionsState(commonSectionKeys, true)}>Select All</Button>
                    ...
                    <InfoCard title="Derived And Vehicle Fields" theme={theme}>
                        <CardText style={{ color: theme.textColorAlt }}>
                            These route cases configure backend generators. Enabling one sends the matching parameters to Flask so it derives that column with the specified distribution.
                        </CardText>
                        <div className="d-flex gap-2 flex-wrap mb-3">
                            <Button color="success" size="sm" onClick={() => setSectionsState(derivedSectionKeys, true)}>Select All</Button>
